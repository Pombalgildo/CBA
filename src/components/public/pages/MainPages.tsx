...
import { sortByDataDesc } from '@/lib/data-parser'

const SITE_URL = 'https://www.cbaangola.org'

export function HomePage({ content }: { content: SiteContent }) {
  const { settings, news, publications, urgentNotices } = content
  // Notícias, publicações e avisos ordenados por data real (mais recente primeiro)
  const newsOrdenadas = sortByDataDesc(news)
  const pubsOrdenadas = sortByDataDesc(publications)
  const avisosOrdenados = sortByDataDesc(urgentNotices)
  const featured = newsOrdenadas.find((n) => n.tipo === 'Notícia') || newsOrdenadas[0]
        ...
        <section className="py-10 md:py-14 bg-[#f0ece4]">
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="space-y-4">
              {avisosOrdenados.slice(0, 3).map((aviso) => (
                  ...
                  <div className="flex flex-col sm:flex-row">
                    ...
                    <div className="p-6 flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-3 flex-wrap">
                        {aviso.link && (
                          <a href={aviso.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm font-body font-semibold text-secondary hover:gap-2 transition-all">
                            📎 Ver documento →
                          </a>
                        )}
                        <a
                          href={`${SITE_URL}/avisos/${aviso.id}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-sm font-body font-semibold text-primary hover:gap-2 transition-all"
                        >
                          🔗 Partilhar →
                        </a>
                      </div>
            ...
            {avisosOrdenados.length > 3 && (
              <div className="text-center mt-6">
                <a href="#/avisos" className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-body font-semibold text-sm uppercase tracking-wider rounded-full px-6 py-2.5 transition-colors shadow-md">
                  Ver todos os avisos ({avisosOrdenados.length}) →
                </a>
              </div>
            )}
          </div>
        ...
        <section className="py-16 md:py-20 bg-gradient-to-b from-white to-muted">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              {pubsOrdenadas.slice(0, 3).map((p) => (
                <a
                  key={p.id}
                  href="#/publicacoes"
                  className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-xl transition-all overflow-hidden flex flex-col group hover:-translate-y-1 duration-300 w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)]"
                >
...
export function PublicacoesPage({ content }: { content: SiteContent }) {
  ...
  // Ordenadas por data real (mais recente primeiro)
  const pubsOrdenadas = sortByDataDesc(publications)
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="mt-6">
              <ReactionsBar
                ...
                itemId={p.id}
                shareUrl={`${SITE_URL}/publicacoes/${p.id}`}
                shareTitle={p.title}
                shareText={p.excerpt}
                shareImage={p.image}
              />
      ...
      <section className="py-16 md:py-24 bg-muted">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pubsOrdenadas.slice(0, visibleCount).map((p: Publication) => (
          ...
          {pubsOrdenadas.length > visibleCount && (
            <div className="text-center mt-12">
              <button
                onClick={() => setVisibleCount((c) => c + PAGE_SIZE)}
                className="inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-white font-body font-semibold text-sm uppercase tracking-wider rounded-full px-8 py-3 transition-colors shadow-md"
              >
                Ver mais publicações ({pubsOrdenadas.length - visibleCount} restantes) →
              </button>
            </div>
          )}
          {visibleCount > PAGE_SIZE && pubsOrdenadas.length <= visibleCount && (
            <div className="text-center mt-10">
              <p className="font-body text-sm text-muted-foreground">
                A mostrar todas as {pubsOrdenadas.length} publicações
              </p>
            </div>
          )}
        </div>
...
export function AvisosPage({ content }: { content: SiteContent }) {
  const { urgentNotices } = content
  // Ordenados por data real (mais recente primeiro)
  const avisosOrdenados = sortByDataDesc(urgentNotices)
  ...
  return (
    <div className="page-enter">
      <section className="bg-white pt-20 pb-12 md:pt-28 md:pb-16">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-2 mb-4"><span className="text-4xl">🚨</span><h1 className="font-heading font-bold text-foreground text-3xl md:text-4xl">Avisos Importantes</h1></div>
          <p className="mt-3 font-body text-muted-foreground text-base">Todos os avisos, anúncios e comunicados da Convenção Baptista de Angola</p>
        </div>
      </section>
      <section className="py-10 md:py-14 bg-[#f0ece4]">
        <div className="max-w-5xl mx-auto px-4">
          {avisosOrdenados.length === 0 ? (
            <div className="text-center py-12">
              <p className="font-body text-muted-foreground">Nenhum aviso disponível de momento.</p>
            </div>
          ) : (
            <>
              <div className="space-y-4">
                {avisosOrdenados.slice(0, visibleCount).map((aviso) => (
                  <div key={aviso.id} className="bg-white rounded-xl shadow-md border-l-4 border-red-600 overflow-hidden">
                    <div className="flex flex-col sm:flex-row">
                      {aviso.imagem && (
                        <div className="sm:w-48 shrink-0">
              ...
              {avisosOrdenados.length > visibleCount && (
                <div className="text-center mt-8">
                  <button
                    onClick={() => setVisibleCount((c) => c + PAGE_SIZE)}
                    className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-body font-semibold text-sm uppercase tracking-wider rounded-full px-8 py-3 transition-colors shadow-md"
                  >
                    Ver mais avisos ({avisosOrdenados.length - visibleCount} restantes) →
                  </button>
                </div>
              )}
              {visibleCount > PAGE_SIZE && avisosOrdenados.length <= visibleCount && (
                <div className="text-center mt-6">
                  <p className="font-body text-sm text-muted-foreground">
                    A mostrar todos os {avisosOrdenados.length} avisos
                  </p>
                </div>
              )}
            </>
