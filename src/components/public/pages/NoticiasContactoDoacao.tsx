...
import { sortByDataDesc, parseDataPT, mesAnoPT } from '@/lib/data-parser'
...
export function NoticiasPage({ content }: { content: SiteContent }) {
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="mt-6">
              <ReactionsBar
                itemType="news"
                itemId={item.id}
                shareUrl={`${SITE_URL}/noticias/${item.id}`}
  ...
  // Ordena por data real (mais recente primeiro) e filtra por categoria
  const todosOrdenados = sortByDataDesc(news)
  const filtrados = filtro === 'Todos' ? todosOrdenados : todosOrdenados.filter((i) => i.tipo === filtro)
  const visiveis = filtrados.slice(0, visibleCount)

  // Agrupa as notícias visíveis por mês/ano para mostrar cabeçalhos
  type Grupo = { mesAno: string; items: NewsItem[] }
  const grupos: Grupo[] = []
  for (const item of visiveis) {
    const ts = parseDataPT(item.data)
    const chave = mesAnoPT(ts)
    const ultimo = grupos[grupos.length - 1]
    if (ultimo && ultimo.mesAno === chave) {
      ultimo.items.push(item)
    } else {
      grupos.push({ mesAno: chave, items: [item] })
    }
  }
      ...
      <section className="bg-background border-b border-border py-5">
        <div className="max-w-6xl mx-auto px-4 flex flex-wrap justify-center gap-2">
          {NEWS_CATEGORIAS.map((cat) => (
            <button
              key={cat}
              onClick={() => { setFiltro(cat); setVisibleCount(PAGE_SIZE) }}
        ...
        <div className="max-w-6xl mx-auto px-4">
          {filtrados.length === 0 && (
            <div className="text-center py-12">
              <p className="font-body text-muted-foreground">Nenhuma notícia encontrada nesta categoria.</p>
            </div>
          )}
            ...
            {grupos.map((grupo) => (
              <div key={grupo.mesAno}>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="font-heading font-bold text-xl text-primary">{grupo.mesAno}</h2>
                  <div className="flex-1 h-px bg-border"></div>
                  <span className="text-xs font-body text-muted-foreground uppercase tracking-wider">
                    {grupo.items.length} {grupo.items.length === 1 ? 'notícia' : 'notícias'}
                  </span>
                </div>
          ...
          {filtrados.length > visibleCount && (
            <div className="text-center mt-12">
              ...
              >
                Ver mais notícias ({filtrados.length - visibleCount} restantes) →
              </button>
            </div>
          )}
          {visibleCount > PAGE_SIZE && filtrados.length <= visibleCount && filtrados.length > 0 && (
            <div className="text-center mt-10">
              <p className="font-body text-sm text-muted-foreground">
                A mostrar todas as {filtrados.length} notícias{filtro !== 'Todos' ? ` na categoria "${filtro}"` : ''}
              </p>
            </div>
          )}
        </div>
