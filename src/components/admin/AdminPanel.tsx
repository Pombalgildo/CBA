...
import { getAdminToken, clearAdminToken, adminList, getAdminUser, refreshAdminUser, hasTabPermission, isSuperAdmin } from '@/lib/api-client'
...
import { EditoresPage } from './EditoresPage'
...
const SIDEBAR_ITEMS = [
  { hash: '#/admin', label: 'Dashboard', icon: '📊', permKey: null },
  { hash: '#/admin/avisos', label: 'Avisos', icon: '🚨', permKey: 'avisos' },
  { hash: '#/admin/noticias', label: 'Notícias', icon: '📰', permKey: 'noticias' },
  { hash: '#/admin/publicacoes', label: 'Publicações', icon: '📖', permKey: 'publicacoes' },
  { hash: '#/admin/eventos', label: 'Galeria', icon: '🖼️', permKey: 'eventos' },
  { hash: '#/admin/igrejas', label: 'Igrejas', icon: '⛪', permKey: 'igrejas' },
  { hash: '#/admin/ministerios', label: 'Departamentos', icon: '🙏', permKey: 'ministerios' },
  { hash: '#/admin/quem-somos', label: 'Quem Somos', icon: '🏛️', permKey: 'quem-somos' },
  { hash: '#/admin/contactos', label: 'Contactos', icon: '📬', permKey: 'contactos' },
  { hash: '#/admin/doacoes-cats', label: 'Doações', icon: '🤲', permKey: 'doacoes-cats' },
  { hash: '#/admin/mensagens', label: 'Mensagens', icon: '📨', permKey: 'mensagens' },
  { hash: '#/admin/doacoes', label: 'Comprovativos', icon: '💰', permKey: 'doacoes' },
  { hash: '#/admin/comentarios', label: 'Comentários', icon: '💬', permKey: '__superadmin__' },
  { hash: '#/admin/editores', label: 'Editores', icon: '👥', permKey: '__superadmin__' },
  { hash: '#/admin/definicoes', label: 'Definições', icon: '⚙️', permKey: 'definicoes' },
]
...
export function AdminPanel() {
  ...
  // Enquanto carrega as permissões actualizadas do backend, mostramos um loading
  ...
  // Ao montar, busca permissões actualizadas do backend (caso tenham mudado após o login)
  useEffect(() => {
    if (!logado) {
    ...
    refreshAdminUser()
      ...
      .catch(() => {
        // Erro de rede — mantém o login mas usa o que está em localStorage
      })
  ...
  // Mostra loading enquanto busca as permissões actualizadas
  if (loadingPerms) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted">
        <div className="text-center">
          <div className="spinner mx-auto mb-4" style={{ borderColor: 'rgba(0,0,0,0.2)', borderTopColor: 'hsl(150 30% 18%)' }}></div>
          <p className="font-body text-sm text-muted-foreground">A carregar permissões...</p>
        </div>
      </div>
    )
  }

  // Helper: verifica se o utilizador actual pode ver um item do menu
  const canSeeItem = (permKey: string | null) => {
    if (permKey === null) return true // Dashboard é sempre visível
    if (permKey === '__superadmin__') return isSuperAdmin()
    return hasTabPermission(permKey)
  }

  // Itens visíveis para o utilizador actual
  const visibleItems = SIDEBAR_ITEMS.filter((item) => canSeeItem(item.permKey))
  // Divide em duas linhas: primeira metade e segunda metade
  const midIdx = Math.ceil(visibleItems.length / 2)
  const firstRow = visibleItems.slice(0, midIdx)
  const secondRow = visibleItems.slice(midIdx)

  // Helper: verifica se o utilizador actual pode aceder à aba actual
  const currentItem = SIDEBAR_ITEMS.find((i) => i.hash === hash)
  const canAccessCurrent = currentItem ? canSeeItem(currentItem.permKey) : true

  // Info do utilizador actual para a navbar
  const currentUserInfo = getAdminUser()
  const displayName = currentUserInfo ? (currentUserInfo.nome || currentUserInfo.username) : '—'
  const displayRole = isSuperAdmin() ? 'Admin' : 'Editor'

  let conteudo: React.ReactNode
  if (!canAccessCurrent) {
    // Acesso negado a esta aba
    conteudo = (
      <div className="bg-amber-50 border border-amber-300 rounded-xl p-8 text-center">
        <h2 className="font-heading font-bold text-xl text-amber-900 mb-2">⚠️ Acesso restrito</h2>
        <p className="font-body text-sm text-amber-800">
          Não tem permissão para aceder a esta secção. Contacte o administrador caso precise de acesso.
        </p>
      </div>
    )
  ...
  } else if (hash === '#/admin/editores') {
    conteudo = <EditoresPage />
  } else if (hash === '#/admin/comentarios') {
    conteudo = <ComentariosPage />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-2">
              {/* Info do utilizador actual */}
              <div className="hidden md:flex items-center gap-2 bg-white/10 rounded px-3 py-1.5 mr-2">
                <span className="text-white/80 text-xs font-body">{displayName}</span>
                <span className={isSuperAdmin() ? 'text-[10px] font-body font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-accent text-primary' : 'text-[10px] font-body font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-white/20 text-white'}>
                  {displayRole}
                </span>
              </div>
          ...
          {/* Linha 2: Primeira metade dos itens visíveis */}
          ...
          {/* Linha 3: Segunda metade dos itens visíveis */}
          {secondRow.length > 0 && (
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const labels: { label: string; icon: string; hash: string; compute?: (items: any[]) => number }[] = []

        if (hasTabPermission('noticias')) {
          promises.push(adminList('news'))
          labels.push({ label: 'Notícias', icon: '📰', hash: '#/admin/noticias' })
        }
        if (hasTabPermission('publicacoes')) {
          promises.push(adminList('publications'))
          labels.push({ label: 'Publicações', icon: '📖', hash: '#/admin/publicacoes' })
        }
        if (hasTabPermission('eventos')) {
          promises.push(adminList('events'))
          labels.push({ label: 'Eventos', icon: '🖼️', hash: '#/admin/eventos' })
        }
        if (hasTabPermission('igrejas')) {
          promises.push(adminList('churches'))
          labels.push({ label: 'Igrejas', icon: '⛪', hash: '#/admin/igrejas' })
        }
        if (hasTabPermission('ministerios')) {
          promises.push(adminList('ministries'))
          labels.push({ label: 'Departamentos', icon: '🙏', hash: '#/admin/ministerios' })
        }
        if (hasTabPermission('mensagens')) {
          promises.push(adminList('messages'))
          labels.push({ label: 'Mensagens Novas', icon: '📬', hash: '#/admin/mensagens', compute: (items) => items.filter((m: any) => !m.lida).length })
        }
        if (hasTabPermission('doacoes')) {
          promises.push(adminList('donations'))
          labels.push({ label: 'Doações Pendentes', icon: '💰', hash: '#/admin/doacoes', compute: (items) => items.filter((d: any) => !d.confirmado).length })
        }
        ...
        const newStats = results.map((items, i) => ({
          label: labels[i].label,
          icon: labels[i].icon,
          hash: labels[i].hash,
          value: labels[i].compute ? labels[i].compute!(items) : items.length,
        }))
        setStats(newStats)
      } catch {}
    }
  ...
  return (
    <div>
      <h2 className="font-heading font-bold text-2xl text-foreground mb-2">
        Olá, {currentUser?.nome || currentUser?.username || 'Administrador'} 👋
      </h2>
      <p className="font-body text-sm text-muted-foreground mb-8">
        {isSuperAdmin()
          ? 'Tem acesso total a todas as secções do painel.'
          : 'Tem acesso limitado às secções atribuídas pelo administrador.'}
      </p>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {stats.length === 0 ? (
          <div className="col-span-full text-center py-8 text-muted-foreground font-body text-sm">
            Sem estatísticas para mostrar com as suas permissões actuais.
          </div>
        ) : (
          stats.map((s) => (
      ...
      <div className="mt-8 bg-white border border-border rounded-xl p-6">
        ...
        <ul className="space-y-2 font-body text-sm text-foreground/80">
          ...
          {isSuperAdmin() && (
            <li className="flex gap-2"><span className="text-accent">▸</span> Em <a href="#/admin/editores" className="text-primary underline">Editores</a> pode criar perfis com acesso parcial ao painel.</li>
          )}
          {hasTabPermission('definicoes') && (
